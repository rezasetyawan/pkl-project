...
export default function PublicRelationHome() {
  ...
  useEffect(() => {
    ...
    getCountFromServer(collection(db,"users")).then(snap=> setUsersCount(snap.data().count))
    getCountFromServer(collection(db,"students")).then(snap=> setStudentsCount(snap.data().count))
    getCountFromServer(collection(db,"companies")).then(snap=> setCompaniesCount(snap.data().count))
  }, [currentHour]);
    ...
    <article className="bg-slate-50 p-5">
      ...
      <div className="my-5">
        ...
        <section className="grid grid-cols-2 gap-3 sm:grid-rows-2">
          ...
          <div className="flex p-2 shadow-md rounded-md items-center hover:scale-[1.02] hover:transition-transform hover:cursor-pointer" onClick={()=> router.push("/public-relation/students")}>
            <div className="w-[50%]">
          ...
          <div className="flex p-2 shadow-md rounded-md items-center hover:scale-[1.02] hover:transition-transform hover:cursor-pointer" onClick={()=> router.push("/public-relation/companies")}>
            <div className="w-[50%]">
